...
(function (global, document) {
  ...
  function initAgendaPage() {
    ...
    // Lista “flat” de agendamentos do dia (do backend), usada para filtros e para montar slots na grade
    ...
    async function carregarAgendaDia() {
      ...
      try {
        ...
        // "achatamos" tudo que veio do backend para uma lista única
        ...
        aplicarFiltrosDia(); // agora monta a grade completa + filtros
      } catch (error) {
        console.error(error);
    ...
    /**
     * Monta slots do dia de acordo com AgendaConfig (grade completa),
     * preenche com agendamentos do backend, e aplica filtros.
     */
    function aplicarFiltrosDia() {
      ...
      const map = new Map(); // key = "HH:MM" => agendamentos[]
      (agendamentosOriginaisDia || []).forEach((ag) => {
        ...
        // filtros
        if (termo) {
          const nome = String(ag.nome_paciente || "").toLowerCase();
          if (!nome.includes(termo)) return;
        }
        if (statusFiltro) {
          const s = String(ag.status || "").toLowerCase();
    ...
    function desenharHorarios(horarios) {
      ...
      horarios.forEach((slot) => {
        ...
        if (!agendamentos || agendamentos.length === 0) {
          ...
          // Atalho: duplo clique abre novo agendamento no horário
