...
import ProntuariosError from "./error";

describe("ProntuariosError", () => {
  it("renderiza ModuleError com props corretas", () => {
    render(<ProntuariosError error={new Error("fail")} reset={vi.fn()} />);
  ...
  it("passa a função reset para ModuleError", async () => {
    const reset = vi.fn();
    render(<ProntuariosError error={new Error("fail")} reset={reset} />);
