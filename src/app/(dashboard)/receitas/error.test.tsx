...
import ReceitasError from "./error";

describe("ReceitasError", () => {
  it("renderiza ModuleError com props corretas", () => {
    render(<ReceitasError error={new Error("fail")} reset={vi.fn()} />);
  ...
  it("passa a função reset para ModuleError", async () => {
    const reset = vi.fn();
    render(<ReceitasError error={new Error("fail")} reset={reset} />);
