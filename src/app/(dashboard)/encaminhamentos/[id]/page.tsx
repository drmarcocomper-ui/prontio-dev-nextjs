...
import { excluirEncaminhamento } from "../actions";
import {
  type Encaminhamento,
  formatDateLong,
  formatPhone,
...
export async function generateMetadata({
  ...
  if (!UUID_RE.test(id)) return { title: "Encaminhamento" };
  ...
  const nome = (data as unknown as { pacientes: { nome: string } } | null)
    ?.pacientes?.nome;
  return { title: nome ? `Encaminhamento - ${nome}` : "Encaminhamento" };
}
...
export default async function EncaminhamentoDetalhesPage({
  ...
  const { data: encaminhamento } = await supabase
    .from("encaminhamentos")
    .select(
      "id, data, profissional_destino, especialidade, telefone_profissional, motivo, observacoes, created_at, pacientes(id, nome)"
    )
    .eq("id", id)
    .single();
  ...
  const e = encaminhamento as unknown as Encaminhamento;
    ...
    <div className="animate-fade-in mx-auto max-w-3xl space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white shadow-sm p-4 sm:flex-row sm:items-start sm:justify-between sm:p-6">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/encaminhamentos/${e.id}/imprimir`}
          ...
          <Link
            href={`/encaminhamentos/${e.id}/editar`}
            className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm transition-colors hover:bg-gray-50"
          >
            <svg aria-hidden="true" className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
          ...
          <DeleteButton onDelete={excluirEncaminhamento.bind(null, e.id)} title="Excluir encaminhamento" description="Tem certeza que deseja excluir este encaminhamento? Esta ação não pode ser desfeita." errorMessage="Erro ao excluir encaminhamento. Tente novamente." />
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-4 sm:p-6">
        ...
        <dl className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            ...
            <dd className="mt-0.5 text-sm text-gray-900">{e.profissional_destino}</dd>
          </div>
          <div>
            <dt className="text-xs font-medium text-gray-500">Especialidade</dt>
            <dd className="mt-0.5 text-sm text-gray-900">{e.especialidade}</dd>
          </div>
          {e.telefone_profissional && (
            <div>
              <dt className="text-xs font-medium text-gray-500">Telefone</dt>
              <dd className="mt-0.5 text-sm text-gray-900">
                <a href={`tel:${e.telefone_profissional.replace(/\D/g, "")}`} className="text-primary-600 hover:text-primary-700 hover:underline">
                  {formatPhone(e.telefone_profissional)}
                </a>
              </dd>
            </div>
          )}
