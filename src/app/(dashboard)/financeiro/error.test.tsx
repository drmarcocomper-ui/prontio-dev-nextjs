...
import FinanceiroError from "./error";

describe("FinanceiroError", () => {
  it("renderiza ModuleError com props corretas", () => {
    render(<FinanceiroError error={new Error("fail")} reset={vi.fn()} />);
  ...
  it("passa a função reset para ModuleError", async () => {
    const reset = vi.fn();
    render(<FinanceiroError error={new Error("fail")} reset={reset} />);
