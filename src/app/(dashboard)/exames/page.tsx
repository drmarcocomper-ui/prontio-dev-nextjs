...
import { ExameFilters } from "./filters";
import {
  type ExameListItem,
  TIPO_LABELS,
  formatDate,
...
export const metadata: Metadata = { title: "Solicitações de Exames" };
...
export default async function ExamesPage({
  ...
  let query = supabase
    ...
    .select("id, data, tipo, exames, pacientes(id, nome)", { count: "exact" })
  ...
  if (q) {
    ...
    query = query.or(`exames.ilike.%${escaped}%,pacientes.nome.ilike.%${escaped}%`);
  }
  ...
  const { data: exames, count, error } = await query;
  ...
  const items = (exames ?? []) as unknown as ExameListItem[];
    ...
    <div className="animate-fade-in space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="min-w-0 flex-1">
          <SearchInput basePath="/exames" placeholder="Buscar por paciente ou exame..." ariaLabel="Buscar solicitações de exames" defaultValue={q} />
        </div>
        <ExameFilters currentTipo={tipo ?? ""} />
      </div>
      ...
      {items.length > 0 ? (
          ...
          {items.map((e) => (
            ...
            >
              <div className="flex items-start gap-4">
                {/* Avatar */}
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-cyan-100 text-xs font-semibold text-cyan-700">
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-white shadow-sm px-6 py-16 text-center">
          <EmptyStateIllustration type="exames" />
          ...
          <p className="mt-1 text-sm text-gray-500">
            {q
              ...
              : "Crie a primeira solicitação de exame."}
          </p>
          {!q && (
            <Link
              href="/exames/novo"
