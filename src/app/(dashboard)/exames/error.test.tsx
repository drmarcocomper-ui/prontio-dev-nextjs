...
import ExamesError from "./error";

describe("ExamesError", () => {
  it("renderiza ModuleError com props corretas", () => {
    render(<ExamesError error={new Error("fail")} reset={vi.fn()} />);
  ...
  it("passa a função reset para ModuleError", async () => {
    const reset = vi.fn();
    render(<ExamesError error={new Error("fail")} reset={reset} />);
