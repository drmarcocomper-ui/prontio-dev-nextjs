...
import { criarLaudo, atualizarLaudo, type LaudoFormState } from "../actions";
...
export function LaudoForm({
  ...
  const action = isEditing ? atualizarLaudo : criarLaudo;

  const [state, formAction, isPending] = useActionState<LaudoFormState, FormData>(
    action,
    {}
  );

  const cancel = cancelHref ?? (isEditing ? `/laudos/${defaults?.id}` : "/pacientes");
    ...
    <form action={formAction} className="space-y-6" aria-busy={isPending}>
      ...
      <div>
        <label htmlFor="conteudo" className="block text-sm font-medium text-gray-700">
          Conteúdo do laudo <span className="text-red-500">*</span>
        </label>
        <textarea
          id="conteudo"
          name="conteudo"
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end border-t border-gray-200 pt-6">
        ...
        >
          ...
          {isEditing ? "Salvar alterações" : "Salvar laudo"}
        </button>
      </div>
    </form>
  );
