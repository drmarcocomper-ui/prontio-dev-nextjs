...
import { excluirAtestado } from "../actions";
import {
  type Atestado,
  TIPO_LABELS,
  formatDateLong,
...
export async function generateMetadata({
  ...
  if (!UUID_RE.test(id)) return { title: "Atestado" };
  const supabase = await createClient();
  let medicoId: string;
  try {
    medicoId = await getMedicoId();
  } catch {
    return { title: "Atestado" };
  }
  ...
  const nome = (data as unknown as { pacientes: { nome: string } } | null)
    ?.pacientes?.nome;
  return { title: nome ? `Atestado - ${nome}` : "Atestado" };
}

export default async function AtestadoDetalhesPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  const { data: atestado } = await supabase
    .from("atestados")
    .select(
      "id, data, tipo, conteudo, cid, dias_afastamento, observacoes, created_at, pacientes(id, nome)"
    )
    .eq("id", id)
    .eq("medico_id", medicoId)
    .single();
  ...
  const a = atestado as unknown as Atestado;
    ...
    <div className="animate-fade-in mx-auto max-w-3xl space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white shadow-sm p-4 sm:flex-row sm:items-start sm:justify-between sm:p-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <DeleteButton onDelete={excluirAtestado.bind(null, a.id)} title="Excluir atestado" description="Tem certeza que deseja excluir este atestado? Esta ação não pode ser desfeita." errorMessage="Erro ao excluir atestado. Tente novamente." />
        </div>
      </div>
      ...
      {(a.cid || a.dias_afastamento) && (
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-4 sm:p-6">
          <h2 className="text-sm font-semibold text-gray-900">Informações adicionais</h2>
          <dl className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {a.cid && (
              <div>
                <dt className="text-xs font-medium text-gray-500">CID</dt>
                <dd className="mt-0.5 text-sm text-gray-900">{a.cid}</dd>
              </div>
            )}
            {a.dias_afastamento && (
              <div>
                <dt className="text-xs font-medium text-gray-500">Dias de afastamento</dt>
      ...
      <p className="text-xs text-gray-400">
        Registro criado em {formatDateTime(a.created_at)}
      </p>
    </div>
  );
}
