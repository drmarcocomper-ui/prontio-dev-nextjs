...
const allSecondaryNavigation = [
  ...
  },
  {
    label: "Auditoria",
    href: "/auditoria",
    roles: ["superadmin", "gestor"] as Papel[],
    icon: (
      <svg aria-hidden="true" className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
  ...
  },
  {
    ...
    roles: ["superadmin", "gestor", "profissional_saude"] as Papel[],
    icon: (
      <svg aria-hidden="true" className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      <div className="border-t border-gray-200 px-4 py-3">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary-100 text-sm font-semibold text-primary-700">
            {getInitials(profissionalNome || userEmail)}
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-gray-900">
              {profissionalNome || userEmail || "Usuário"}
            </p>
            {PAPEL_BADGE[papel] && (
              <span className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium leading-none ${PAPEL_BADGE[papel].className}`}>
                {PAPEL_BADGE[papel].label}
              </span>
            )}
          </div>
          <button
