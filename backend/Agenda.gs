...
function handleAgendaAction(action, payload) {
  ...
  switch (action) {
    ...
    // ✅ NOVO (opcional, útil pro front validar antes de salvar)
 ...
 * Função exigida pelo helper AgendaConflitos.gs.
 *
 * Retorna eventos do dia no formato:
 * [
 *   { ID_Agenda, data, hora_inicio, duracao_minutos, bloqueio, status, permite_encaixe }
 * ]
 *
 * Observação:
 * - Aqui NÃO aplicamos regras de "cancelado/faltou" — isso fica na regra de negócio
 *   que chama Agenda_AssertSemConflitos_ (ver agendaCriar_/agendaAtualizar_/agendaBloquearHorario_).
 */
...
function agendaCriar_(payload) {
  ...
  // ✅ REGRA ÚNICA de sobreposição: usa helper AgendaConflitos.gs (se existir)
  // Mantemos seu comportamento atual:
  // - Se permitir encaixe: não valida conflito com outras consultas, mas continua respeitando bloqueios.
  if (typeof Agenda_AssertSemConflitos_ === 'function') {
    // 1) sempre respeita bloqueio (mesmo com encaixe)
    ...
    // Fallback: mantém suas validações antigas
...
function agendaAtualizar_(payload) {
  ...
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) {
    throw { code: 'AGENDA_EMPTY', message: 'Não há registros na agenda.' };
  }
...
function agendaBloquearHorario_(payload) {
  ...
  // ✅ bloqueio NUNCA pode sobrepor nada (consulta ou bloqueio)
  if (typeof Agenda_AssertSemConflitos_ === 'function') {
    Agenda_AssertSemConflitosComPolitica_({
      data: payload.data,
      inicio: payload.hora_inicio,
...
function agendaListarSemana_(payload) {
  ...
  // ✅ Correção: segunda a sábado = 6 dias (0..5)
  for (var i = 0; i < 6; i++) {
...
function agendaMudarStatus_(payload) {
  ...
  var sheet = getAgendaSheet_();
  var lastRow = sheet.getLastRow();
  if (lastRow < 2) throw { code: 'AGENDA_EMPTY', message: 'Não há registros na agenda.' };
 ...
 * ============================================================
 * A AgendaConflitos.gs detecta sobreposição “bruta”.
 * Aqui aplicamos as regras:
 * - Bloqueio conflita sempre com tudo.
 * - Consulta:
 *    - se permiteEncaixe = true: ignora conflito com outras consultas,
 *      mas NÃO ignora bloqueio.
 *    - ignora consultas canceladas/faltou na hora de impedir sobreposição.
 */
function Agenda_AssertSemConflitosComPolitica_(args, opts) {
  ...
  // Se for bloqueio: qualquer conflito impede
  if (modoBloqueio) {
    throw {
  ...
  // Se for consulta:
  // - sempre bloqueio impede
  // - consulta impede somente se não for encaixe e se o conflito não for cancelado/faltou
  var data = String(args.data || '');
  var eventosDia = Agenda_ListarEventosDiaParaValidacao_(data);

  // mapa ID_Agenda -> status/bloqueio
  var meta = {};
  ...
  // Se houver QUALQUER bloqueio nos conflitos -> impede sempre
  ...
  // Se permite encaixe: conflitos com consultas não impedem
  if (permitirEncaixe) return r;

  // Se NÃO permite encaixe: conflitos com consultas impedem, exceto cancelado/faltou
  for (var c2 = 0; c2 < r.conflitos.length; c2++) {
    ...
    if (m2.status === 'cancelado' || m2.status === 'cancelada' || m2.status === 'falta' || m2.status === 'faltou') {
      continue;
    }
...
/* ============================================================
 * As funções antigas de conflito foram mantidas (fallback).
 * Se AgendaConflitos.gs estiver presente, elas deixam de ser usadas.
 * ============================================================
 */
...
/**
 * ✅ Função utilitária que faltava no seu arquivo (estava sendo usada):
...
function addMinutesToTime_(horaInicioStr, minutesToAdd) {
  ...
  // normaliza para 0..1439 (sem virar data)
